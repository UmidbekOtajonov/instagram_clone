...
def check_email_or_phone_number(email_or_phone_number):
    phone_number = phonenumbers.parse(email_or_phone_number)
...
def check_user_type(user_input):
    ...
    if re.fullmatch(email_regex, user_input):
        ...
        data ={
            "success": False,
            "message": "username, telefon_raqam yoki email noto'g'ri"
        }
        raise ValidationError(data)
    return user_input
...
def send_phone_code(phone_number, code):
    ...
    client.messages.create(
        body=f" Salom do'stim . Sizning tasdiqlash qodingiz: {phone_number}",
        from_="+998919952545",
        to=f"{phone_number}"
