...
from shared.utility import check_email_or_phone_number, send_email, send_phone_code, check_user_type
...
class SignUpSerializer(serializers.ModelSerializer):
    ...
    email_or_phone_number = serializers.CharField(required=True, write_only=True)
    ...
    def auth_validate(data):
        user_input = data.get('email_or_phone_number')
    ...
    def auth_validate(self, data):
        ...
        if current_user is not None and current_user.auth_status in [NEW, CODE_VERIFIED]:
            raise ValidationError(
                {
                    "success":False,
                    "message": "Siz ro'yxatdan to'liq o'tmadingiz "
                }
            )
        ...
        if user is not None:
            ...
            {
                "success":False,
                "message":"Sizning username yoki passwordingiz noto'g'ri, tekshirib qaytadan kiriting"
            }
        )
...
class ForgotPasswordSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        user = User.objects.filter(Q(phone_number=email_or_phone_number) |Q(email__iexact=email_or_phone_number))
        if not user.exists():
