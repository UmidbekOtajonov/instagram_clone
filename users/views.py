...
class GetNewVerification(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if user.auth_type== VIA_EMAIL:
            ...
            code = user.create_verify_code(user.phone_number)
            send_email(user.phone_number, code)
        else :
            data = {
                'message':"email yoki telefon raqam noto'ri"
            }
            raise ValidationError(data)
    ...
    def check_verification(user):
        ...
        if verifies.exists():
            data = {
                'message': 'kodingiz xali ishlatishga yaroqli, iltimos kuting'
...
class ForgotPasswordView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if check_email_or_phone_number(email_or_phone_number) == 'phone_number':
            code = user.create_verify_code(VIA_PHONE)
            send_email(email_or_phone_number, code)
        elif check_email_or_phone_number(email_or_phone_number) == 'email':
            code = user.create_verify_code(VIA_EMAIL)
            send_email(email_or_phone_number, code)
        return Response(
            {
                'success': True,
                'message': 'tasdiqlash kodi muaffaqiyatli yuborildi',
                'access_token':user.token()['access_token'],
                'refresh_token':user.token()['refresh_token'],
...
class ResetPasswordView(UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        return Response(
            {
                'success': True,
                'message': "paro'lingiz muvaffaqiyatli o'zgartirildi ",
                'access_token': user.token()['access_token'],
                'refresh_token': user.token()['refresh_token'],
            }
        )
